...
| **M5.3** | **Live timecode → deck (first scratch)** | Wire `AudioInput` → `dub-timecode::Decoder` → engine deck in **relative mode**: decoded rate becomes deck `set_rate`, deck position seeks from decoded position deltas. CLI: `dub timecode --device <SL3> --deck a`. **Demo criterion: scratch a record on Deck A, hear Deck A's loaded MP3 react with sub-buffer-size latency.** This is the milestone where Dub becomes a DJ app. | 2–3 days |
| **M5.4** | **Stickiness + scope + confidence** | Lift policy: when amplitude / confidence drops below a calibrated threshold for ≥ N ms, freeze the deck (don't drive it from noise). Resume tracking when the carrier re-locks. Simple TUI scope: Lissajous of input `(L, R)` + decoded rate / position / confidence bars. | 2–3 days |
