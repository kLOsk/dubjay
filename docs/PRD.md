...
| **M5.4.4** | **Per-deck calibration** | M5.6 ships two-deck timecode but probes pair 0 (deck A) only and silently reuses those thresholds for deck B. Correct when both turntables have matched cartridges (the common case in vinyl-DJ rigs) but wrong when they don't — and the wrongness is silent: deck B just runs with mis-tuned `LiftPolicy` thresholds, which manifests as ghost-noise on lift or premature disengage on light scratches. Fix: probe and calibrate **each deck independently** on startup, store thresholds per deck, fingerprint per deck. Calibration JSON keys by `(device, deck_index, format)` instead of `(device, format)`; the M5.4.2 fingerprint-probe / auto-recalibrate machinery runs once per deck (in parallel where the AU allows). The CLI prints separate `timecode A:` and `timecode B:` lines so the user can see they're not borrowed. **Explicitly out of scope** (deferred to v1.x or never): named cartridge profiles, profile libraries, cross-session "auto-load the right cartridge" UX. The earlier M5.4.4 design ("library of named profiles, fingerprint-match across them") was over-scoped — once M5.4.3 makes calibration ≤ 5 s, "always recalibrate on startup" is the simpler model with zero profile-management UX surface, and matches the user's mental model ("calibrate auto-runs on app start; if I swap a cartridge mid-set I press the calibrate button"). The "calibrate button" is part of M10 (UI); on the CLI today, `dub calibrate` already serves that role. | 1–2 days |
...
| **M6** | **Timecode v2 (Traktor)** | Traktor MK2 also works. 33+45 RPM, A+B side, abs/rel modes, calibration UI. Note that M5.6's two-deck infrastructure is format-agnostic — adding a second decoder with `Format::TraktorMk2` to deck B's `TimecodeInputConfig` is mostly a per-deck CLI flag away. | 1 week |
